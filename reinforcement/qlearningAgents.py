...
class QLearningAgent(ReinforcementAgent):
    ...
    def computeActionFromQValues(self, state):
        ...
        highestQ = max(qValues)
        indices = [i for i in range(len(legalActions)) if self.getQValue(state, legalActions[i]) == highestQ]  
...
class ApproximateQAgent(PacmanQAgent):
    ...
    def getQValue(self, state, action):
        ...
        for feature, value in features.items():
          dotProd += self.weights[feature] * value
        return dotProd
    ...
    def update(self, state, action, nextState, reward):
        ...
        for feature in features.keys():
          weights[feature] += self.alpha * ((reward + self.discount * self.getValue(nextState)) - self.getQValue(state, action))
